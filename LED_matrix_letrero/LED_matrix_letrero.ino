...
*/
  ...
  int data = 12;    // Pin digital para el pin DIN del módulo MAX7219
  int load = 10;    // Pin digital para el pin CS del módulo MAX7219
  int clock = 11;  // Pin digital para el pin CLK del módulo MAX7219
 
  int maxInUse = 4;    // Cambie este valor dependiendo del número de matrices que conectes (máx.8)
  ...
  char mensaje1[] = "SKINET te da la bienvenida... "; // Escriba el primer mensaje a desplegar
  // Escriba el segundo mensaje a desplegar
  char mensaje2[] = "www.spartadev.com ...";
  char mensaje3[] = "... The future is now ...";
  ...
  void setup()
  {
    ...
    m.init(); // Inicializa el módulo MAX7219
    m.setIntensity(1); // Intensidad luminosa de los puntos de la matriz (entre 1-5)
  }
 
  void loop()
  {
    delay(1000); // Tiempo de inicio de un nuevo Ciclo de mensajes
    m.shiftLeft(false, true); // Desplazamiento del texto hacia la izquierda
   
    // Despliega los mensajes almacenados en las variables  
    printStringWithShift(mensaje1, 100); // El último termino se usa para la velocidad del mensaje
    printStringWithShift(mensaje2, 100);
    printStringWithShift(mensaje3, 100);
  }
